...
if __name__ == '__main__':
    ...
    parser.add_argument('estimated_file1', help='estimated trajectory file (format: "timestamp tx ty tz qx qy qz qw")')
    parser.add_argument('estimated_file2', help='estimated trajectory file (format: "timestamp tx ty tz qx qy qz qw")')
    ...
    traj_est1 = read_trajectory(args.estimated_file1)
    traj_est2 = read_trajectory(args.estimated_file2)
    ...
    result2 = evaluate_trajectory(traj_gt,
                                 traj_est2,
                                 int(args.max_pairs),
                                 args.fixed_delta,
                                 float(args.delta),
    ...
    stamps2 = numpy.array(result2)[:, 0]
    trans_error2 = numpy.array(result2)[:, 4]
    rot_error2 = numpy.array(result2)[:, 5]
    ...
    if args.verbose:
        ...
        print "File 1: RPE trans.rmse %f m"%numpy.sqrt(numpy.dot(trans_error,trans_error) / len(trans_error))
        print "File 1: RPE trans.mean %f m"%numpy.mean(trans_error)
        print "File 1: RPE trans.median %f m"%numpy.median(trans_error)
        print "File 1: RPE trans.std %f m"%numpy.std(trans_error)
        print "File 1: RPE trans.min %f m"%numpy.min(trans_error)
        print "File 1: RPE trans.max %f m"%numpy.max(trans_error)

        print "File 1: RPE rotation.rmse %f deg"%(numpy.sqrt(numpy.dot(rot_error,rot_error) / len(rot_error)) * 180.0 / numpy.pi)
        print "File 1: RPE rotation.mean %f deg"%(numpy.mean(rot_error) * 180.0 / numpy.pi)
        print "File 1: RPE rotation.median %f deg"%(numpy.median(rot_error) * 180.0 / numpy.pi)
        print "File 1: RPE rotation.std %f deg"%(numpy.std(rot_error) * 180.0 / numpy.pi)
        print "File 1: RPE rotation.min %f deg"%(numpy.min(rot_error) * 180.0 / numpy.pi)
        print "File 1: RPE rotation.max %f deg"%(numpy.max(rot_error) * 180.0 / numpy.pi)
        print "File 2: RPE trans.rmse %f m" % numpy.sqrt(numpy.dot(trans_error2, trans_error2) / len(trans_error2))
        print "File 2: RPE trans.mean %f m" % numpy.mean(trans_error2)
        print "File 2: RPE trans.median %f m" % numpy.median(trans_error2)
        print "File 2: RPE trans.std %f m" % numpy.std(trans_error2)
        print "File 2: RPE trans.min %f m" % numpy.min(trans_error2)
        print "File 2: RPE trans.max %f m" % numpy.max(trans_error2)

        print "File 2: RPE rotation.rmse %f deg" % (numpy.sqrt(numpy.dot(rot_error2, rot_error2) / len(rot_error2)) * 180.0 / numpy.pi)
        print "File 2: RPE rotation.mean %f deg" % (numpy.mean(rot_error2) * 180.0 / numpy.pi)
        print "File 2: RPE rotation.median %f deg" % (numpy.median(rot_error2) * 180.0 / numpy.pi)
        print "File 2: RPE rotation.std %f deg" % (numpy.std(rot_error2) * 180.0 / numpy.pi)
        print "File 2: RPE rotation.min %f deg" % (numpy.min(rot_error2) * 180.0 / numpy.pi)
        print "File 2: RPE rotation.max %f deg" % (numpy.max(rot_error2) * 180.0 / numpy.pi)
    else:
        print "File 1: RPE trans.mean %f m" %numpy.mean(trans_error)
        print "File 1: RPE rotation.mean %f deg"%(numpy.mean(rot_error) * 180.0 / numpy.pi)
        print "File 2: RPE trans.mean %f m" % numpy.mean(trans_error2)
        print "File 1: RPE rotation.mean %f deg" % (numpy.mean(rot_error2) * 180.0 / numpy.pi)
    ...
    if args.plot:    
        ...
        ax.plot(stamps - stamps[0],rot_error*180.0 / numpy.pi,'-',color="blue", label = "File 1")
        ax.plot(stamps2 - stamps2[0], rot_error2*180.0 / numpy.pi, '-', color="green", label ="File 2")
        #ax.plot([t for t,e in err_rot],[e for t,e in err_rot],'-',color="red")
        ax.legend(loc='upper right',ncol=5)
        ax.set(aspect=1.0 / ax.get_data_ratio() * 0.4)
        ax.set_xlabel('time [s]')
        ax.set_ylabel('roation error [deg]')
        plt.savefig(args.plot+'_rota.png', dpi=600)

        fig = plt.figure()
        bx = fig.add_subplot(111)
        bx.plot(stamps - stamps[0],trans_error,'-',color="blue", label = "File 1")
        bx.plot(stamps2 - stamps2[0], trans_error2, '-', color="green", label="File 2")
        # ax.plot([t for t,e in err_rot],[e for t,e in err_rot],'-',color="red")
        bx.legend(loc='upper right', ncol=5)
        bx.set_xlabel('time [s]')
        bx.set_ylabel('translational error [m]')
        plt.savefig(args.plot+'_trans.png',dpi=600)
